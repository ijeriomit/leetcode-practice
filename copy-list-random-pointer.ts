...
function copyRandomList(head: ListNode | null): ListNode | null {
    ...
    while(curr?.next != null){
        ...
        nodeArray.push([newNode, curr.next, curr.random]);
        curr = curr.next;
    }
    curr = head;
    let i =0;
    while(curr?.next) {
        let newNode = nodeArray[i][0];
        newNode.next = nodeArray[i][1];
        newNode.random = nodeArray[i][2];
        curr = curr.next;
        i++;
    }
...
printListNodes([[[7,null],[13,0],[11,4],[10,2],[1,0]], [[1,1],[2,1]], [[3,null],[3,0],[3,null]]], copyRandomList);
